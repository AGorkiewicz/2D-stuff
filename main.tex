...
We define $\Q_1^+$ by adding to $Q_1$ the neighbouring $30$ degrees of $\Q_2$ and $\Q_4$, formally
%
$$\Q_1^+ = \{ z \in \mathbb Z^2 \mid (z.x + z.y) \geq \sqrt{2} \cdot \cos(5\pi/12) \cdot \absolute{z} \},$$
%
$$\Q_2^- = \{ z \in \mathbb Z^2 \mid (z.y - z.x) > \sqrt{2} \cdot \cos(\pi/12) \cdot \absolute{z} \}.\phantom{5}$$
Analogously, we obtain the extended $\Q_3$ as $\Q_3^+ = \{ z \in \mathbb Z^2 \mid -z \in \Q_1^+ \}$, and the restricted $\Q_2$ and $\Q_4$ as $\Q_2^- = \Q_2 \setminus (\Q_1^+ \cup \Q_3^+)$ and  $\Q_4^- = \Q_4 \setminus (\Q_1^+ \cup \Q_3^+)$. We define two relations $\le_1$ and $\le_2$ over $U$, where, for every $u \in U$, it holds $u \le_1 u$ and $u \le_2 u$ (i.e., the relation is \emph{reflexive}). For $u, v \in U$ with $u \neq v$, it holds $u \le_1 v$ if and only if $(v - u) \in \Q_1^+$, and $u \le_2 v$ if and only if $(v - u) \in \Q_2^-$.
Note that two distinct points $u,v \in U$ are comparible with $\le_1$ if and only if $(v - u) \in \Q_1^+ \cup \Q_3^+$, and they are comparible with $\le_2$ if and only if $(v - u) \in \Q_2^- \cup \Q_4^-$, where $(\Q_1^+ \cup \Q_3^+)$ and $\Q_2^- \cup \Q_4^-$ are a bipartition of $\mathbb Z^2 \setminus (0,0)$. Hence they are comparible with $\le_1$ if and only if they are not comparible with $\le_2$.
In a moment, we will show that $\le_1$ and $\le_2$ are partial orders. By the previous argument, every antichain with respect to $\le_1$ is a chain with respect to $\le_2$ and vice versa.
...
%
where the last inequality holds by design of $w$. In a moment, we will show that we can compute the longest chain $C$ and antichain $A$ with respect to $\le_{w}$ in $\tO(\absolute{U})$ time. Now we show that, given $A$ and $C$, we can find the vector $w'$ with the desired properties in $\tO(U)$ time.
...
thus $|A| \ge 2$.
We select any pair of different vectors $s', t' \in A$ that minimizes $|t' - s'|$. W.l.o.g., we assume $s'.x \geq t'.x$ (otherwise swap $s'$ and $t'$), and we construct $w' = t' - s'$.
