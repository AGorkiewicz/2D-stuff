...
We first partition $F_1''$ into $\ell$ strings $V_i$, $\ell = \O(m/d)$ by \cref{lm:partitioning}. For all $q \in Q$, we have 
...
For all $q \in Q$, it holds $\d{P_0 + q} \cap \d{V_i} = \emptyset$.
\end{claim}
\begin{proof}
