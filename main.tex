...
Note that because $|Q| > 2m + m^2 / k$, we have $0 \le \phi \times \psi \le |\phi||\psi| = \O(\min\set{m, k})$.
...
We partition $R$ into a set of subparquet strings $\S$.

For every non-empty $S \in \S$ we construct a lattice graph $\Graph(\d{S})$. If $S$ is not monochromatic, then since $G_{\d{S}}$ is connected, there must exist a pair of neighboring vectors $v, w$ such that $S(v) \neq S(w)$.
