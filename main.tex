...
Assume to be given a (truncated) tile string $R$ such that $\phi, \psi$ are its $\O(k)$-periods. After $\O((\min\set{k,m})^2)$-time preprocessing of $\phi, \psi$, the string $R$ can be partitioned in time $\tO(|\d{R}| + k)$ into $\O(k)$ monochromatic (truncated) subtile strings.
\end{restatable*}
...
% tile DECOMPOSITION
\subsection{Properties of lattices and truncated subtiles} \label{tile_decomposition_proof}
...
\begin{lemma} \label{lattice_base}
	There exists an algorithm that constructs in $\O((\min\set{k,m})^2)$ time a set of points $\Gamma \subseteq \Z^2$ such that $|\Gamma| = \O(\min\set{m, k})$ and $\forall u \in \Z^2 \; \exists! \gamma \in \Gamma : u \equiv \gamma$.
\end{lemma} 
	\begin{proof}
		...
		We now give a construction algorithm for $\Gamma$. Consider $\Gamma' = \Z^2 \cap [0,\x{\phi}+\x{\psi}] \times [\y{\psi},\y{\phi}]$. We have $|\Gamma'| = \O((|\x{\phi}|+|\x{\psi}|) \cdot (|\y{\phi}|+|\y{\psi}|)) = \O((\min\set{k,m})^2)$. For $u \in \Gamma'$ we have $u \in \Gamma$ iff $0 \le \s{u} \le \s{\phi}$ and $0 \le \h{u} \le \h{\psi}$, which can be checked in constant time. 
	\end{proof}
...
Given a truncated subtile string $S$. There is an algorithm that runs in time $\tO(|\d{S}| + 1)$ and constructs two partitionings:
	...
	\begin{proof}
		...
		If $S$ is a (truncated) subtile string, then the created strings are (truncated) subtile strings as well.
