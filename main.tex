...
\SparseAlgo
Let $U = \bigcup_{S \in \S} \d{S}$. Recall that $\V$ is a set of monochromatic truncated subtile strings which partition $P$ and $\V_a = \set{V : V \in \V, \getchar{V} = a}$ (see \ref{sec:partitioning}).
 Observe that
\begin{align}
\label{eq:sparse}
...
Denote $D = \set{u : u \in \L, \h{u} \ge 0, \s{u} \ge 0}$, where $\L$ is the set defined in \Cref{lattice_congruency} (geometrically, $D$ is the part of the lattice spanned by the angle with the sides collinear to $\phi$ and~$\varphi$).
