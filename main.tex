...
%
$$\Q_2^- = \{ (x,y)\phantom{\ \setminus \{(0,0)\}} \in \mathbb Z^2 \mid y > -x / \sqrt{3}\text{ and }y < -x \cdot \sqrt{3}\},$$
%
...
We define two relations $<_1$ and $<_2$ over $U$. For $u, v \in U$, it holds $u <_1 v$ if and only if $(v - u) \in \Q_1^+$, and $u <_2 v$ if and only if $(v - u) \in \Q_2^-$. In a moment, we will show that both $\le_1$ and $\le_2$ are strict partial orders. 
Note that two distinct points $u,v \in U$ are comparible with $\le_1$ if and only if $(v - u) \in \Q_1^+ \cup \Q_3^+$, and they are comparible with $\le_2$ if and only if $(v - u) \in \Q_2^- \cup \Q_4^-$. 
...
Now we show that $<_1$ is a strict partial order (the proof for $\le_2$ works analogously). First, note that $(0,0 \notin \Q_1^+$, and thus it cannot be that $u <_1 u$, i.e., the relation is \emph{irreflexive}. It is easy to see that the intersection of $\Q_1^+$ and $\Q_3^+$ is empty. This means that, if $u <_1 v$, then it cannot also be that $v <_1 u$, i.e., the relation is \emph{asymmetric}. Finally, given $u <_1 v$ and $v <_1 v'$, it holds $(v.y - u.y) \geq -(v.x-u.x)/\sqrt{3}$ and $(v'.y - v.y) \geq -(v'.x-v.x)/\sqrt{3}$ by the definition of $\Q_1^+$. By summing the inequalities, we get $(v'.y - u.y) \geq -(v'.x-u.x)/\sqrt{3}$. The same reasoning applys to the second inequality of the definition, und thus it holds $(v' - u) \in \Q_1^+$ and therefore $u < v'$, i.e., the relation is \emph{transitive}.

We have shown that $<_1$ is a strict partial order. We will show how to compute the longest chain $C$ and antichain $A$ with respect to $<_1$ in $\tO(\absolute{U})$ time. Now we show that, given $A$ and $C$, we can find the vector $w'$ with the desired properties in $\tO(U)$ time.
...
$12 \ell \le |U| \le |C| |A| < 12\ell|A|$,
thus $|A| \ge 2$.
We select any pair of different vectors $s', t' \in A$ that minimizes $|t' - s'|$. Since $s'$ and $t'$ are in an antichain with respect to $<_1$, they are in a chain with respect to $<_2$. Hence either $s' <_2 t'$ or $t' <_2 s'$. W.l.o.g., we assume $s' <_2 t'$ (otherwise swap $s'$ and $t'$), and we construct $w' = t' - s'$, which is in $\Q_2^-$ as required.
Next, we show $|w||w'| = \O(\ell^2 / |U|)$.
