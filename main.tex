...
In \todo{section}, we show that the Hamming distances between $P$ and a string consisting of a monochromatic subtile strings can be computed efficiently: 
...
We then select some special periods of the pattern and show how to decompose it into some regularly structured strings.
...
\begin{definition}[Lattice congruency]\label{lattice_congruency}
Define $\L = \set{s\phi + t\psi : s, t \in \Z}$, called a lattice. We say that $u, v \in \Z^2$ are \textbf{lattice-congruent}, denoted $u \equiv v$, if $u - v \in \L$.
\end{definition}
...
\begin{definition}[Subtile]\label{subtile_definition}
	We call a set $V \subseteq \Z^2$ a \emph{(truncated) subtile} when there exists a tile $U$ and a point $\gamma \in \Z^2$ such that
	\[ V = \set{u : u \in U, u \equiv \gamma}.\]
	This is also illustrated in \Cref{figure:tile}.
	A signature of $V$ consists of a signature of $U$ and the vector $\gamma$.
	We call $V$ a spacious/simple subtile when there exists $U$ which is (correspondingly) a spacious/simple tile.
	We say that $V$ is lattice-congruent to some $v \in \Z^2$ (denoted as $V \equiv v$) when $v \equiv \gamma$.
	We similarly define the lattice congruency between two subtiles.
\end{definition}


\begin{definition}[tile string]\label{tile_string_definition}
	We call a string $S$ a spacious/simple (sub-)tile string when $\d{S}$ is a spacious/simple (sub-)tile.
\end{definition}


\begin{restatable*}{theorem}{tileDecomposition}\label{tile_decomposition}
	A given spacious/simple tile string $R$ with $\O(k)$-periods $\phi$ and $\psi$ can be partitioned in time $\tO(|\d{R}| + k)$ into $\O(k)$ monochromatic spacious/simple subtile strings, correspondingly.
\end{restatable*}
\begin{proof} See \Cref{tile_decomposition_proof}. \end{proof}


Since $|\x{\phi}|, |\y{\phi}|, |\x{\psi}|, |\y{\psi}| \le n - m \le m / 2$, the $m \times m$ string $P$ is a spacious tile string and satisfies the assumptions of \Cref{tile_decomposition}.
...
\begin{restatable*}{theorem}{SparseAlgo}\label{sparse_algo}
	For a given set of monochromatic simple subtile strings $\S$ we can calculate
...
\end{restatable*}
\begin{proof} See \Cref{subtile_convolution_proof}. \end{proof}
...
\begin{lemma}\label{monochromacy_condition}
	...
	\begin{proof}
		...
		Since $\d{S}$ is a spacious subtile, the graph $(\d{S}, \Edges(\d{S})$ is connected (by \Cref{lattice_graph_connectivity}) and there must exist a path between $u$ and $v$.
...
\begin{proof}
	...
	We now partition each $S' \in \S'$ by using \Cref{cut_partitioning} (\ref{partition_b}) and construct a set of subtile strings $\S''$, such that $\S''$ partitions $R$ and $\Ham(S'' + \psi, S'') = 0$ for every $S'' \in \S''$.
	Again we have
	\[ |S''| = \sum_{S' \in \S'} \O(\Ham(S' + \psi, S') + 1) = \O(\Ham(R + \psi, R) + |\S'|) = \O(k),\]
	since $R$ has an $\O(k)$-period $\psi$.
	The process is illustrated in \Cref{figure:tile_decomposition}.
