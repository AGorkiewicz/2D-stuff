...
We define $\Q_1^+$ by adding to $Q_1$ the neighbouring $30$ degrees of $\Q_2$ and $\Q_4$, and restrict $\Q_2$ to $\Q_2^-$ by limiting it to points strictly contained in its central 30 degrees, formally
...
\path (-10, \line) to node[midway] {$\Q_2^-$} (-\line, 10);
\path (10, -\line) to node[midway] {$\Q_4^-$} (\line, -10);
\node at (5,5) {$\Q_1^+$};
\node at (-5,-5) {$\Q_3^+$};
...
\draw[-Latex] (t1.center) to (t0.center);
\draw[-Latex] (t3.center) to (t2.center);
\draw[-Latex] (t5.center) to (t4.center);

\node at (l1) {\rotatebox{-15}{\rlap{\ $30^{\circ}$}}};
\node at (l2) {\rotatebox{-45}{\rlap{\ $30^{\circ}$}}};
\node at (l3) {\rotatebox{-75}{\rlap{\ $30^{\circ}$}}};
