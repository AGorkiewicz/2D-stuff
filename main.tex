...
Given a one-dimensional string $S$ of size $\ell$ such that none of its characters is a wildcard. There is a data structure that can be built in $\O(\ell)$ times and for all pairs of strings $S_1 = S(i) S(i+1) \ldots S(i+\ell')$, $S_2 = S(j) S(j+1) \ldots S(j+\ell')$, where $1 \le i,j \le \ell-\ell'$, allows computing $\MI(S_1, S_2)$ in time $\O(\Ham(S_1,S_2))$. 
\end{fact}
...
Given two non-empty two-dimensional strings $P$ and $T$ of widths $w_P, w_T$ and heights $h_P, h_T$, we can solve the text-to-pattern Hamming distances problem in time $\O((\sigma+1) \cdot (w_P + w_T)(h_P + h_T) \log (w_P + w_T)(h_P + h_T))$, where $\sigma$ is the number of different characters present both in $P$ and $T$.
\end{corollary}
\begin{proof}
...
Let $\varepsilon > 0$ be a constant. Given two non-empty two-dimensional strings $P$ and $T$ of widths $w_P, w_T$ and heights $h_P, h_T$, there is a $(1+\varepsilon)$-approximation algorithm that solves the text-to-pattern Hamming distances problem in time $\O(((w_P+w_T)(h_P+h_T)/\varepsilon^2)  \log^3 m)$.
\end{corollary}
\begin{proof}
...
Below, we focus on the text-to-pattern Hamming distances and the $k$-mismatch problem for two-dimensional strings $P$ and $T$, where $\dom{P} = [m] \times [m]$ and $\dom{T} = [n] \times [n]$ for $m, n \in \Z^+$. 
	...
	\subsection{Decomposition of the pattern}

	\subsection{Decomposition of the text}

	\subsection{Algorithm}
Recall that we focus on the text-to-pattern Hamming distances and the $k$-mismatch problem for two-dimensional strings $P$ and $T$, where $\dom{P} = [m] \times [m]$ and $\dom{T} = [n] \times [n]$ for $m, n \in \Z^+$. Assume w.l.o.g. that $2|n$ and $n \le \frac{3}{2}m$. If this is not the case, one can cover $T$ with $\O(n^2/m^2)$ strings $T_i$ such that $\d{T_i} = [n'] \times [n'] + q$ for $n' = \Theta(m)$ and $q \in \Z^2$ and solve the problem for $P$ and each $T_i$ independently.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Decomposition of the pattern}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Decomposition of the text}
...
\begin{proof}
	...
	By \Cref{subparquet_definition}, $I_u$ are simple subparquet strings, and thus, we can by \Cref{sparse_algo} calculate the results in $\tO(m^2 + \sum_{u \in \d{I}} |\V_{I(u)}|)$.
