...
\begin{abstract}
We consider a natural generalisation of the classical approximate pattern matching problem to two-dimensional strings.
A two-dimensional string is simply a square array of characters. Given two such arrays, the pattern of size $m\times m$
and the text of size $n\times n$, our goal is to find all locations in the text where the pattern match with at most $k$
mismatches. This problem has been extensively studied for regular one-dimensional strings, and by now we have a good
understanding of the best possible time complexity as a function of $n$, $m$, and $k$. In particular, we know that
for $k=\O\sqrt(m)$ we can achieve quasi-linear time complexity [Gawrychowski and Uznański, ICALP 2018]. Surprisingly,
no similar statement is known for two-dimensional strings, as the asymptotically fastest algorithm works in $\O(kn^{2})$
time [Amir and Landau, TCS 1991]. We improve on this bounds from 30 years ago with a non-trivial adaptation of tools
used to tackle the one-dimensional version, and design an $\tO((m^{2}+k^{5/4}m)n^{2}/m^{2})$ time algorithm.
...
The basic algorithmic problem considered in the context of sequences of characters, called strings, is pattern matching:
...
data structure for longest common extensions, such as a suffix tree augmented with a lowest common ancestors structure,
...
the both bounds were unified to obtain an $\O(n\sqrt{k \log k})$  time algorithm~\cite{Amir2004}.  This complexity
...
of allowing Monte Carlo randomisation). In a very recent exciting improvement, it was shown how to slightly improve
these time complexities by leveraging a connection to the 3-SUM problem~\cite{Chan0WX23}. Thus, it appears
that the time complexity of one-dimensional pattern matching with bounded Hamming distance is fairly well understood.
...
for this problem was obtained already in the late 70s~\cite{Bird1977}, but obtaining such a complexity without
...
\paragraph{Periodicities in 2D strings.} The fundamental combinatorial tool used for 1D strings is that of periodicity,
defined as follows. We say that $p$ is a period of $s[1..n]$ when $s[i]=s[i+p]$, for all $i$ such that the expression is defined.
...
becomes more involved~\cite{Amir1998}, but still remains to be a powerful tool for exact pattern matching~\cite{Amir1992,Galil1996}.
Some purely combinatorial properties of two-dimensional periodicities have been studied~\cite{Mignosi2003,Gamard2017},
but generally speaking repetition in two-dimensional strings are inherently more complicated than in one-dimensional strings.
...
setting~\cite{Baeza-Yates1998a}, the natural challenge is to obtain a better understanding of the complexity
...
The starting point for our algorithm is the algorithm for the one-dimensional version, see e.g.~\cite{Gawrychowski2018}
for an optimised version (but the approach is due to~\cite{Clifford2015}), which proceeds as follows.
First, we approximate the Hamming distance for every position in the ext, by adapting Karloff's~\cite{Karloff1993}
method to 2D. Then, we can eliminate positions for which the approximated distance is very large.
If the number of remaining position is small enough, we can essentially adapt the kangaroo jump~\cite{Galil1986}.
Otherwise, some two remaining possible occurrences must have a large overlap, and thus induce a periodicity in the pattern.
Then, the two-dimensional case becomes significantly more involved (due to the more complex nature of two-dimensional periodicities).
To construct a functional compressed representation of a periodic string, we require it to have not one, but two periods.
For technical reasons, the periods (which we associate with vectors) have to be selected with great care and their existance allows us to decompose the pattern into some regularly structured monochromatic fragments.
The number of such fragments is $\O(k)$ and the positions each of them is defined for are given by some lattice of points restricted to some polygon.
We call such a set of positions a subparquet.
After successfully dealing with the pattern, we proceed with the text.
Although the whole text is not periodic, some of its fragments are similar to the pattern and share the same periods.
Because of that we can use a modified decomposition approach for the area covered by those fragments specifically.
Since the complexity of the algorithm depends on how fast we can convolve the pairs of monochromatic fragments that we decompose the pattern and the text into, we need to construct .
The time needed to aggregate the contribution of a single pair of subparquets is $\tO(1)$, but only in the case when one of them is simple, which translates to an additional condition regarding its structure.
Accordingly, the text needs to be decomposed into fragments defined for simple subparquets.
Since the number of such fragments would be too large if we considered them exclusively, we instead introduce a notion of a peripheral set of positions.
The positions in the peripheral set can overlap with only a small portion of the positions in the pattern, and considering them provides us with an alternative speed-up method.
We show how to decompose the pattern into fragments, which are either monochromatic and defined for simple subparquets, or peripheral ones.
With various approaches we then calculate their contributions to the number of mismatches and sum them for each occurrence.
...
We can construct such partitioning by substituting a large enough value $\ell = \Theta(m)$, such that the obtained $\O(m / \ell)$-peripheral string is in fact $0$-peripheral, and thus empty.
...
To obtain the final $\tO(m^2 + mk^{5/4})$ complexity, we partition the active text using the algorithm from \Cref{text_decomposition} with $\ell = mk^{-3/4}$.
...
In the next section (\ref{periphery_section}) we will introduce \Cref{dense_algo}, which states that for a $d$-peripheral string $F$, we can calculate $\Ham(P + q, F)$ for every $q \in Q$ in total time $\tO(m^2 + mdk^{1/2})$.
...
\begin{observation}\label{area_bound}
	...
	By \Cref{sum_of_h} we have $|\d{S_1}| = \O(md)$ and by extension $|\d{S}| = \O(md)$.
\end{observation}
