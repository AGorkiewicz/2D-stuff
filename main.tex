...
\jonasi{below}\noindent%
Next, we use $\phi$ and $\psi$ to define tiles, which will be used to partition $P$ and $T$ into one-character strings of regular form.
The partitioning will depend on the lattice spanned by $\phi$ and $\psi$. We use the lattice to partition the domains of $P$ and $T$ into $\O(\min\{m, k\})$ congruency classes.
...
\subsection*{Monochromatic tiles.}
As mentioned before, we will partition $P$ and $S$ into one-character strings of regular form. A string containing only a single character is \emph{monochromatic, formally:}
...
\subsection{Properties of lattices and subtiles}
