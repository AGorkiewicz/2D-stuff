...
To compute $|\d{P + q} \cap U|$ for all $q \in Q$, we create strings $P'$ and $T'$ with respective domains $\dom(P)$ and $\dom(T)$. Both strings are filled with character $0$. 
In $T'$, we then replace all positions from $\dom(U)$ with character $1$. 
We observe that $|\d{P + q} \cap U| = \Ham(P' + q, T')$, which can be computed for all $q \in Q$ in $\tO(m^2)$ time with \cref{cor:sigman2d}.
...
We apply \Cref{primitive} to each $S\in \S$ to obtain in $\O(|\S|)$ total time $w_{i, j}(S)$, where $i,j \in \{0,1\}$. We then have:
