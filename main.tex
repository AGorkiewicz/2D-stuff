...
We then select the occurrences, for which the predicted results are small enough to be calculated precisely.
...
Although the text is not periodic, because some of its fragments are similar to the pattern, we can use a modified decomposition approach for the area covered by those fragments specifically.
After decomposing the pattern and the text, we then convolve the resulting pairs of monochromatic fragments and aggregate their contributions.
