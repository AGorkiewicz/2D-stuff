...
\begin{restatable*}{theorem}{TextDecomposition}\label{text_decomposition}
	For a given positive integer $\ell \le m$, we can partition the active text in time $\tO(m^2 + \ell k)$ into a set of $\O(\ell k)$ monochromatic simple subparquet strings and an $\O(m / \ell)$-peripheral string.
\end{restatable*}
...
\subsubsection{Periphery decomposition} \label{sigma_border_proof}

This section serves as the proof of the following theorem:
...
Specifically, we introduce some methods of partitioning them into monochromatic strings.
...
For better reference, the decomposition can be seen in \Cref{figure:text_decomposition}.
...
This section serves as the proof of \Cref{parallelogram_split_lemma}.
...
Consider any $p_{i, j}$ for some $i, j \in [\ell]$.
