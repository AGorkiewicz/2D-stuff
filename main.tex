...
which gives a smooth trade-off between $\tO(n\sqrt{k})$ and $\tO(n + k^2n/m)$\footnote{We use $\tO$ to hide factors polylogarithmic in $n$.}. 
...
We follow a pipeline similar to the \hd solution described in \cite{Gawrychowski2018}. 
Our method first approximates the Hamming distance for every occurrence of the pattern in the text by using a two-dimensional variant of the algorithm introduced in \cite{Karloff1993}.
We then select the occurrences, for which the predicted result is small enough to be calculated precisely.
If we have few such occurrences, for each of them we explicitly discover every mismatch position with the text, by using a two dimensional variant of the kangaroo jumps technique introduced in \cite{Galil1986}.
In the other case, that is, when we have a lot of such occurrences, we take advantage of the fact that some of them must have a large overlap and consequently, the pattern must be periodic.
