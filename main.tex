...
Considering \Cref{AB_size_bound}, we can partition the set $A$ into $\ell$ sets $A_0, \dots, A_{\ell - 1}$, such that each $A_i$ contains some subsequent integers ($A_0$ the smallest, $A_{\ell - 1}$ the largest) and $|A_i| \le \lceil 2n|\phi| / \ell \rceil$.
Specifically, 
...
For every $i, j \in [\ell]$ we construct the simple parquet (recall \Cref{parquet_definition})
$$ U_{i,j} = \set{u : u \in \Z^2, \h{u} \in A_i, \s{u} \in B_j}.$$
We will use the \Cref{width_and_height_definition} of the width and height of a set.
