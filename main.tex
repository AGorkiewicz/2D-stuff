...
% AG: For now I just remove "and only if" since it makes the definition less incorrect
Let $\prec$ be a strict total order defined by $u \prec v$ if $u <_2 v$.
We compute the longest subsequence in the sorted $U$ that is increasing with respect to $\prec$.
This takes $\tO(\absolute{U})$ time (see~\cite{CROCHEMORE20101054} and references therein) and results in the longest chain of $<_2$, or equivalently the longest antichain of $<_1$.
\end{proof}
