...
Efficient linear-time algorithms for this problem are known since the 70s~\cite{Knuth1977}, and dozens 
...
As an immediate corollary, we obtain an $\tO(m^2 + mk^2)$-time solution for the $k$-mismatch problem. First, we apply \Cref{text_decomposition} for a large enough value $\ell = \Theta(m)$ to partition the active text $\Ta$ into a set $\S$ of $\O(mk)$ monochromatic subtile strings and a $0$-peripheral string, empty by definition, in time $\tO(m^2 + mk)$. For every $q \in Q$ we then have $\Ham(P + q, \Ta) = \sum_{S \in \S} \Ham(P + q, S)$, and these values can be computed in $\tO(m^2 + mk^{2})$ total time by \cref{sparse_algo}, since $\sum_{S \in \S} \absolute{\V_{\getchar{S}}} \le \absolute{\S} \absolute{\V} = \O(mk \cdot k)$. 
...
\end{lemma}
\begin{proof}
By the definition of $\Ta$, there does not exist $u \in \Z^2 \cap K_1 \setminus \d{\Ta}$ and $v \in \d{\Ta}$, such that $\x{u} \le \x{v}$ and $\y{u} \le \y{v}$. From this, \Cref{lm:parallelogram_grid} (\ref{it:monotonicity}), and \ref{coverable_peripheral}, for every $j \in [\ell]$ there exist $s,s', t',t \in [\ell]$  such that each of the following is satisfied:
\begin{enumerate}
...
Denote $D = \set{u : u \in \L, \h{u} \ge 0, \s{u} \ge 0}$, where $\L$ is the set defined in \Cref{lattice_congruency} (geometrically, $D$ is the part of the lattice spanned by the angle with the sides collinear to $\phi$ and~$\psi$).
...
\[ \absolute{U \cap X} = \sum_{i,j \in \{0,1\}} (-1)^{(i+j)} \absolute{(D + w_{i,j}) \cap X}\]
...
\end{enumerate}
(Note that $\forall u \in D + w_{i,j}$, we have $u \equiv \gamma$). We can compute $w_{i,j}$ in $\O(1)$ time given $U$'s signature and if $\max \{\absolute{u} : u \in U\} = \O(m)$, then $\absolute{w_{i,j}} = \O(m)$ as well. 
\end{observation}

\todo{figure would be good}

We apply \Cref{primitive} to each $S\in \S$ to obtain in $\O(\absolute{\S})$ total time $w_{i, j}(S)$, where $i,j \in \{0,1\}$. We then have:
...
\begin{equation}
\label{eq:fixed_ij}
\tau_q^{i,j} = (-1)^{(i+j)} \sum_{S \in \S} \sum_{V \in \V_{\getchar{S}}} \absolute{\d{D-q} \cap \d{V-w_{i, j}(S)}}
\end{equation}

Below we explain how to compute the values $\tau_q^{i,j}$ for all $q \in Q$ and all $i,j \in \{0,1\}$ in time $\tO(m^2 + \sum{S \in \S} \absolute{\V_{\getchar{S}}})$. It is easy to see from \cref{eq:S_replaced_by_shifted_angles} that the values $\tau_q$ can then be computed in $\O(\absolute{Q}) = \O(m^2)$ time. 

Define a set of truncated subtiles $\S \odot \V := \{V-w_{i, j}(S) : S \in \S, V \in \V_{\getchar{S}}\}$.  Note that $\absolute{\S \odot \V} = \O(\sum{S \in \S} \absolute{\V_{\getchar{S}}})$ and for all $u \in \S \odot \V$, we have $\absolute{u} = \O(m)$. We can then simplify \cref{eq:fixed_ij} as follows:
...
There is an algorithm that computes the values $\tau_q^{i,j}$ for all $q \in Q$ in total time $\tO(m^2 + \sum{S \in \S} \absolute{\V_{\getchar{S}}})$. 
\end{lemma}
\begin{proof}
...
\begin{split}
&\tau_q^{i,j} =\\
&\sum_{V \in \S \odot \V} \absolute{u \in \Z^2 : u \in (D-q), u \in V} = \sum_{V \in \S \odot \V} \sum_{u \in D-q}  \absolute{u \in V} =  \\
&\sum_{u \in D-q} \sum_{V \in \S \odot \V} \absolute{u \in V} = \sum_{u \in D-q} \score(u)
\end{split}
\end{equation}
We now explain how to compute $\tau_q^{i,j}$. We start by computing in $\O(\S \odot \V) = \tO(\sum{S \in \S} \absolute{\V_{\getchar{S}}})$ time the maximum distance $\ell$ from the origin to a vertex of the polygon bounding $V \in \S \odot \V$. Note that $\ell$ is 
an upper bound on the maximum length of $u \in V$, $V \in \S \odot \V$. On the other hand, by \cref{primitive} and as $\V$ is a partitioning of $P$, we have $\ell = \O(m)$. We therefore can construct the set $\W = \{w \in \Z^2 : \absolute{w} \le \ell\}$ in $\O(m^2)$ time. Since for $w \notin \W$ we have $\score(w) = 0$, we only need to compute the scores for $w \in \W$. 

We then construct in $\O(m^2)$ the set $\Gamma$ defined in \Cref{lattice_base} and for each $\gamma \in \Gamma$ define $\W_\gamma = \{w \in W : w \equiv \gamma\}$. For $\gamma \in \Gamma$, let $(\S \odot \V) |_\gamma = \{V \in \S \odot \V : V \equiv \gamma\}$. By \cref{subtile_definition}, that if $w \in \W_\gamma$ is contained in a truncated subtile $V \in \S \odot \V$, then $V \equiv \gamma$, and consequently $V \in (\S \odot \V) |_\gamma$. Therefore, for $w \in \W_\gamma$ we can refine the definition of score:
%
$$\score(w) = \sum_{V \in (\S \odot \V) |_\gamma} \absolute{u \in V}$$
%
...
\end{equation}
(we don't need to check whether $w \equiv \gamma$ as $w \in \W_\gamma$). We build in $\tO(\absolute{(\S \odot \V) |_\gamma})$ time a $6$-dimensional range tree~\cite{BENTLEY1979244} for the set of points 
%
$$\{(x_0, x_1, \phi_0, \phi_1, \psi_0, \psi_1) : x_0, x_1, \phi_0, \phi_1, \psi_0, \psi_1, 'gamma \text{ is a signature of } V \in V \in (\S \odot \V) |_\gamma\}$$
Using the range tree, it then follows from \cref{eq:range} that we can compute $\score(w)$ for every $w \in \W_\gamma$ in time $\tO(\absolute{\W_\gamma})$. 
