...
For all $\ell \in \Z^+$, we can construct in $\O(\ell)$ time two sequences of lines defining a parallelogram grid $p_{i,j}$, where $i,j \in [\ell+1]$, which satisfies each of the following:
\begin{enumerate}[(a)]
...
\appendix
\section{Parallelogram grid}
\todo{Rewrite this}
\begin{observation}\label{line_existence}
	For all $\ell \in \Z^+$ and $v \in \Z^2$ we can find a sequence of parallel lines $f_0, f_1, \dots, f_\ell$, where $f_i = \set{u : u \in \R^2, v \times u = c_i}$ for some $c_i$, such that
	\begin{itemize}
