...
Let $S$ be a subtile with signature $\phi_0, \phi_1, \psi_0, \psi_1, \gamma$ and $X \subseteq \Z^2$. There exist $w_{i,j} \in \Z^2$, $i,j \in \{0,1\}$ such that
\[ \absolute{S \cap X} = \sum_{i,j \in \{0,1\}} (-1)^{(i+j)} \absolute{(D + w_{i,j}) \cap X}\]
...
Below we explain how to compute the values $\tau_q^{i,j}$ for all $q \in Q$ and all $i,j \in \{0,1\}$ in time $\tO(m^2 + \sum_{S \in \S} \absolute{\V_{\getchar{S}}})$. Then, the values $\tau_q$ can be computed in $\O(\absolute{Q}) = \O(m^2)$ time because $\tau_q = \tau^{0,0}_q - \tau^{0,1}_q - \tau^{1,0}_q + \tau^{1,1}_q$ by \cref{eq:S_replaced_by_shifted_angles,eq:fixed_ij}.

Define a set of truncated subtiles $\S \odot \V := \{V-w_{i, j}(S) : S \in \S, V \in \V_{\getchar{S}}\}$.  Note that $\absolute{\S \odot \V} = \O(\sum_{S \in \S} \absolute{\V_{\getchar{S}}})$, and for all $u \in V$ with $V \in \S \odot \V$, we have $\absolute{u} = \O(m)$. We can then simplify \cref{eq:fixed_ij} as follows:
...
There is an algorithm that computes the values $\tau_q^{i,j}$ for all $q \in Q$ in total time $\tO(m^2 + \sum_{S \in \S} \absolute{\V_{\getchar{S}}})$. 
\end{lemma}
\begin{proof}
...
\end{split}
\end{equation}
We now explain how to compute $\tau_q^{i,j}$. We start by computing in $\O(\S \odot \V) = \tO(\sum_{S \in \S} \absolute{\V_{\getchar{S}}})$ time the maximum distance $\ell$ from the origin to a vertex of the polygon bounding $V \in \S \odot \V$. Note that $\ell$ is 
