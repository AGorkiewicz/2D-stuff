...
\author[1]{Adam Górkiewicz}
\author[2]{Paweł Gawrychowski}
\author[3]{Tatiana Starikovskaya}
\author[4]{Jonas Ellert}
\affil[1]{?}
\affil[2]{?}
\affil[3]{?}
\affil[4]{?}
...
\newcommand{\hd}{\textsc{1DASHD}\xspace}
\defproblem
{\textsc{All-substring Hamming distance (\hd)}}
{String $T$ of length $n$ and string $P$ of length $m \le n$.}
{The Hamming distance between $P$ and every substring of $T$ that has length $m$.}


\renewcommand{\pm}{\textsc{1DPMWM}\xspace}
\defproblem
{\textsc{Pattern matching with $k$ mismatches (\pm)}}
{String $T$ of length $n$, string $P$ of length $m \le n$, integer $k \ge 0$.}
{The same as for \hd, but if any distance is larger than $k$, the result for that substring should equal $\infty$.}


\begin{fact}
	Pattern matching with $k$ mismatches can be solved in $\tO((m + k \sqrt{m})n / m)$ time.
\end{fact}


\newcommand{\HD}{\textsc{2DASHD}\xspace}
\defproblem
{\textsc{Two-dimensional all-substring Hamming distance (2DASHD)}}
{2D string $T$ of size $n \times n$, 2D string $P$ of size $m \times m$ where $m \le n$.}
{The Hamming distance between $P$ and every $m \times m$ substring of $T$.}


\newcommand{\PM}{\textsc{2DPMWM}\xspace}
\defproblem
{\textsc{Two-dimensional pattern matching with $k$ mismatches (2DPMWM)}}
{2D string $T$ of size $n \times n$, 2D string $P$ of size $m \times m$ where $m \le n$, integer $k \ge 0$.}
{The same as for \HD, but if any distance is larger than $k$, the result for that substring should equal $\infty$.}
