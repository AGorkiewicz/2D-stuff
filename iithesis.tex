...
\englishtitle{Two-dimensional pattern matching \fmlinebreak with $k$ mismatches}
\polishtitle{Wyszukiwanie dwuwymiarowego wzorca z $k$ niezgodnościami}
\englishabstract{
...
\polishabstract{
	Rozważamy naturalne uogólnienie klasycznego problemu przybliżonego wyszukiwania wzorca w tekście do dwuwymiarowych napisów.
	Dwuwymiarowy napis to po prostu kwadratowa tablica znaków.
	Mając dwie takie tablice -- wzorzec o rozmiarze $m \times m$ oraz tekst o rozmiarze $n \times n$ -- naszym celem jest znalezienie wszystkich fragmentow tekstu, w których występuje wzorzec z~co najwyżej $k$ niezgodnościami.
	Problem ten był dotychczas głównie rozważany dla jednowymiarowych napisów i jego optymalna złożoność czasowa jako funkcja $n$, $m$ i $k$ została szeroko zbadana.
	W szczególności, wiemy, że dla $k = \O(m)$, możemy osiągnąć złożoność prawie liniową [Gawrychowski i Uznański, ICALP 2018].
	Co ciekawe, podobne stwierdzenie nie zostało pokazane dla napisów dwuwymiarowych, a najszybszy znany algorytm działa w czasie $\O(kn^2)$ [Amir i Landau, TCS 1991].
	Nasze rozwiązanie poprawia tę znaną od 30 lat złożoność, stosując niebanalną adaptację narzędzi używanych do rozwiązania przypadku jednowymiarowego i~działa w czasie $\tO((m^2 + mk^{5/4})n^2/m^2)$.
	Innymi słowy, nasz algorytm ma złożoność $\tO(n^2)$ dla $k = \O(m^{4/5})$.
	Wyniki odpisane w tym licencjacie zostały uzyskane podczas współpracy Jonasa Ellerta, Pawła Gawrychowskiego, Adam Górkiewicza i Tatiany Starikovskayi , a na ich podstawie powstanie wspólna publikacja.
}


\author{Adam Górkiewicz}

\advisor{dr Paweł Gawrychowski, prof. UWr}
\date{\today} % Data zlozenia pracy
\transcriptnum{321989} % Numer indeksu
...
\chapter{One-dimensional generalizations}
...
% 2D PERIODICITY
\section{Two-dimensional periodicity} \label{periodicity_section}
...
See \Cref{figure:occurrences} for an illustration.
...
If we denote the resulting set of monochromatic simple parquet strings as $\S$, for every $q \in Q$ we have
...
% BORDER PROXIMITY
\section{Text periphery}\label{periphery_section}
\begin{figure}[!t]
	\begin{center}
...
\subsection{Peripheral convolution} \label{sigma_border_proof}
...
% PHI PSI FINDER
\section{Period acquisition} \label{get_periods_proof}
...
\section{Subparquet convolution} \label{subparquet_convolution_proof}
...
\section{Periodic parquet partitioning} \label{parquet_decomposition_proof}
...
% TEXT DECOMPOSITION
\section{Active text decomposition} \label{text_decomposition_proof}
...
\subsection{Parallelogram splitting} \label{parallelogram_split_lemma_proof}
...
\subsection{Parallelogram span bounds} \label{distance_bound_lemma_proof}
